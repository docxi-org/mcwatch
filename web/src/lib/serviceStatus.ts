  ...
  const load = (): Promise<void> => {
    pending ??= fetchStatus()
      ...
      .catch(() => {
        ...
        // обходе — украшение, а не условие показа списка. Но снимок остаётся
        // `null`, и экраны обязаны говорить «неизвестно», а не «не было»:
        // это разные вещи.
        status.value = null;
      })
      .finally(() => {
        // Обещание не запоминается навсегда: неудачная попытка должна
        // допускать следующую, иначе один сбой сети врал бы до перезагрузки.
        pending = null;
      });
    return pending;
  };
